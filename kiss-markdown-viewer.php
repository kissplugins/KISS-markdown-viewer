...
define( 'KISS_MDV_RENDERER_URL',  plugin_dir_url( __FILE__ ) );
...
if ( ! class_exists( 'Parsedown' ) ) {
    require_once KISS_MDV_RENDERER_PATH . 'vendor/parsedown/Parsedown.php';
}
 ...
 */
function kiss_mdv_render_file( $markdown_file ) {
     ...
     * Other plugins can hook: add_filter('wp_kiss_mdv_renderer_html', function($html){ return $modified; });
     */
    return apply_filters( 'wp_kiss_mdv_renderer_html', $html, $markdown_file );
}
 ...
 */
function wp_kiss_mdv_renderer_add_admin_page() {
    add_options_page(
        'KISS Markdown Viewer Settings',
        'KISS MD Viewer',
        'manage_options',
        'kiss-md-viewer-settings',
        'wp_kiss_mdv_renderer_admin_page'
    );
}
add_action( 'admin_menu', 'wp_kiss_mdv_renderer_add_admin_page' );

/**
 * Module 4.1: Admin page callback
 */
function wp_kiss_mdv_renderer_admin_page() {
    echo '<div class="wrap"><h1>KISS Markdown Viewer</h1>';
    $file = KISS_MDV_RENDERER_PATH . 'README.md';
 ...
 */
function wp_kiss_mdv_renderer_add_settings_link( $links ) {
    $settings_link = '<a href="options-general.php?page=kiss-md-viewer-settings">' . __( 'Settings' ) . '</a>';
    array_unshift( $links, $settings_link );
    return $links;
}
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'wp_kiss_mdv_renderer_add_settings_link' );

// -----------------------------------------------------------------------------
// 5. Front-End Shortcode
// -----------------------------------------------------------------------------
/**
 * Module 5: Shortcode [kiss_mdv_render file=""] for front-end use
 *
 * Usage: echo do_shortcode('[kiss_mdv_render file="path/to/file.md"]');
 */
function wp_kiss_mdv_renderer_shortcode( $atts ) {
    $atts = shortcode_atts( [ 'file' => '' ], $atts, 'kiss_mdv_render' );
    $path = KISS_MDV_RENDERER_PATH . ltrim( $atts['file'], '/' );

    $html = kiss_mdv_render_file( $path );
    if ( is_wp_error( $html ) ) {
        return '<p>' . esc_html( $html->get_error_message() ) . '</p>';
    }
    return '<div class="md-content">' . wp_kses_post( $html ) . '</div>';
}
add_shortcode( 'kiss_mdv_render', 'wp_kiss_mdv_renderer_shortcode' );
